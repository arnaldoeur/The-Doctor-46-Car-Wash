...
const buildPdfInstance = async (document: BusinessDocument): Promise<jsPDF> => {
	...
	pdf.setFont('helvetica', 'bold');
	pdf.setFontSize(24);
	pdf.text(kindLabels[document.kind] || document.kind.toUpperCase(), 14, 49);
	pdf.setFontSize(11);
	pdf.setFont('helvetica', 'normal');
	pdf.text(document.title || 'Documento Oficial', 14, 56);
	...
	pdf.setFont('helvetica', 'normal');
	pdf.text(`Número: ${document.number}`, 18, 124);
	pdf.text(`Emissão: ${document.issueDate}`, 74, 124);
	...
	if (document.kind !== 'letterhead') {
		...
		const bodyLines = pdf.splitTextToSize(
			document.body ||
				'Documento institucional pronto para ofícios, comunicados e correspondência oficial da empresa.',
			182
		);
...
export const generateBusinessDocumentPdf = async (document: BusinessDocument) => {
	const pdf = await buildPdfInstance(document);
	pdf.save(`${document.number}.pdf`);
};

export const getBusinessDocumentPdfUrl = async (document: BusinessDocument): Promise<string> => {
	const pdf = await buildPdfInstance(document);
