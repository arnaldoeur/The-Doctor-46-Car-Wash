...
export default function ProtectedRoute({ children }: { children: ReactNode }) {
 ...
 <p className="text-sm uppercase tracking-[0.24em] text-primary">Portal do Cliente</p>
 <h1 className="mt-3 font-display text-2xl font-bold text-white">
 A validar a sua sessao...
 </h1>
 </div>
 </div>
 );
 ...
 return <Navigate to={`/login?redirect=${redirectTarget}`} replace state={{ from: location }} />;
 }
